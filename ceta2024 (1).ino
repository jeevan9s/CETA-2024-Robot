 ...
 *
 * File:            running-the-fairway-collision-detect.ino
 * Project:         Tech Briefs #3 - Add Collision Detection to the RPi Pico W Robot (19-00030A)         
 * Date:            April 28, 2023
 * 
 * "Running the Fairway" Demo with Collision Detection
 * 
 * Robot implements the "Running the Fairway - Collision Detect" challenge
 *
...
#define OPTO_SAMPLE_INTERVAL 2          // sample interval (in mS) for sampling the sensors
...
#define COLLISION_DETECT_THRESHOLD 25.0 

// Servo Motor Default Speed Settings (Straight Motion)
#define LEFT_SERVO_DEFAULT_STRAIGHT 22    
#define RIGHT_SERVO_DEFAULT_STRAIGHT 21    
#define LSERVO_UPPER_SPEED_LIMIT (LEFT_SERVO_DEFAULT_STRAIGHT + 5)
#define LSERVO_LOWER_SPEED_LIMIT (LEFT_SERVO_DEFAULT_STRAIGHT - 10)
#define RSERVO_UPPER_SPEED_LIMIT (RIGHT_SERVO_DEFAULT_STRAIGHT + 5)
#define RSERVO_LOWER_SPEED_LIMIT (RIGHT_SERVO_DEFAULT_STRAIGHT - 10)
...
void Initialize(void){
  ...
  Serial.begin(115200);
  Serial.println("Tech Briefs #3: \"Running the Fairway\" Demo with Collision Detect\r\n");
...
void RobotTurnAround(void){
  ...
  g_lServoSpeed = LSERVO_UPPER_SPEED_LIMIT;
  g_rServoSpeed = RSERVO_UPPER_SPEED_LIMIT;
  RobotRotateRight(g_lServoSpeed, g_rServoSpeed); // begin a sharp turn
  delay(700);                                    // ensure optos are not positioned over line                                      
  g_lServoSpeed = LSERVO_LOWER_SPEED_LIMIT;
  g_rServoSpeed = RSERVO_LOWER_SPEED_LIMIT;
  RobotRotateRight(g_lServoSpeed, g_rServoSpeed);
